...
def load_dataset():
    ...
    with open("aggregate.bin", "rb") as f:
        ...
        for i in range(n):
            ...
            bitboards.append(fields[:12])

    return bitboards

games = load_dataset()

for i, bbs in enumerate(games):
    if i % 200 != 0:
        continue
...
sns.kdeplot(old_scores, label="Old data")
sns.kdeplot(new_scores, label="New data (NNUE)")
plt.xlabel("Score")
plt.ylabel("Density")
